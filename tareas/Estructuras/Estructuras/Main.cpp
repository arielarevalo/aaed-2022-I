...
void rbtree_test(vector <string>& puntos)
{
	try
	{
		cout << "Creando arbol..." << endl;
		rbtree<int>* tp = new rbtree <int>;
		cout << "Creacion exitosa!" << endl;
	...
	try
	{
		...
		while (true)
		{
			...
			if (chrono::steady_clock::now() - time_start1 > chrono::seconds(20))
			{
				break;
			}
		}
	...
	try
	{
		...
		{
			int random = generateRandomNumber(0, 2 * elements_q);
			rbnode<int>* search_for2 = t2->iterativeTreeSearch(random);
			searched_elements++;
			if (search_for2 != NULL)
			{
				a = search_for2->key;
	...
	try
	{
		rbnode <int>* su1 = t2->treeSuccessor(b);
		cout << "El sucesor de " << b->key << " es: " << su1->key << endl;
		puntos.push_back("Sucesor (2)");
	}
	catch (const std::exception& e)
...
void hash_test(vector <string>& puntos)
{
	...
	try
	{
		...
		hasht<int>* h3 = new hasht<int>(elements_q);
		cout << "Creacion exitosa!" << endl;
		puntos.push_back("Constructor (3)");
	}
	catch (const std::exception& e)
	...
	cout << "\nTest #1: Tabla generada aleatoria" << endl;

	cout << "Insertando elementos en la tabla..." << endl;

	hasht<int>* h1 = new hasht<int>(elements_q);
	try
	{
		for (int i = 0; i < elements_q; i++)
		{
	...
	try
	{
		...
		{
			int random = generateRandomNumber(0, 2 * elements_q);
			int* search_for = h1->search(random);
			searched_elements++;
			if (chrono::steady_clock::now() - time_start1 > chrono::seconds(20))
			{
				break;
	...
	cout << "\nTest #2: Tabla secuencial" << endl;

	hasht<int> h2{elements_q};
	searched_elements = 0;
	...
	try
	{
		cout << "Operacion de delete sobre tabla: h1" << endl;
		delete h1;
		cout << "Delete exitoso!" << endl;
		puntos.push_back("Destructor (3)");
	}
